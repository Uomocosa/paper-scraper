...
from paper_scraper.__global__ import EXTRACTED_REFERENCES, DOWNLOADED_DIR, SEED_DIR
...
class Config():
    ...
    download_reference_opts: DownloadReferenceOptions = field(default_factory=DownloadReferenceOptions)
...
def download_papers(config: Config) -> None:
    ...
    OpenAlex.download_papers_from_dois(dois, config.openalex_opts)
    ...
    downloaded_papers = list(DOWNLOADED_DIR.glob("*.pdf"))
    if downloaded_papers:
        logger.info(f"Found {len(downloaded_papers)} papers in DOWNLOADED/")
        ...
        OpenAlex.download_papers_from_dois(reference_dois, config.openalex_opts)
